...
export class GeoClient {
    ...
    handleError(error) {
        if (axios.isAxiosError(error)) {
            ...
            throw new DataSourceError(`Geo API error: ${axiosError.message}`, "GEO", axiosError.response?.status || 502);
        }
        throw new DataSourceError(error instanceof Error ? error.message : "Unknown Geo error", "GEO");
    }
    ...
    getRegionByName(name) {
        ...
        return this.regions.find((r) => r.name.toLowerCase() === normalizedName ||
    ...
    getCityByName(name) {
        const normalizedName = name.toLowerCase().trim();
        return this.cities.find((c) => c.name.toLowerCase() === normalizedName ||
     ...
     */
    getNearestCities(latitude, longitude, limit = 5) {
        const citiesWithDistance = this.cities.map((city) => ({
            ...city,
            distance: this.calculateDistance({ latitude, longitude }, { latitude: city.coordinates[1], longitude: city.coordinates[0] }),
        }));
     ...
     */
    calculateDistance(from, to, unit = "km") {
     ...
     */
    calculateDistanceAndBearing(from, to, unit = "km") {
        return {
            from,
            to,
     ...
     */
    getMoroccoBoundingBox() {
        return { ...this.moroccoBBox };
    }
    /**
     * Check if coordinates are within Morocco
     */
    isWithinMorocco(latitude, longitude) {
        return (latitude >= this.moroccoBBox.minLat &&
     ...
     */
    createPointFeature(longitude, latitude, properties) {
        return {
            type: "Feature",
            geometry: {
                type: "Point",
     ...
     */
    createFeatureCollection(features, properties) {
        return {
     ...
     */
    async getCitiesGeoJSON() {
        const features = this.cities.map((city) => ({
            type: "Feature",
            id: city.id,
            geometry: {
                type: "Point",
     ...
     */
    getAdminHierarchy(regionCode, provinceCode, communeCode) {
    ...
    searchLocations(query) {
        ...
        const results = [];
        // Search cities
        const matchingCities = this.cities.filter((c) => c.name.toLowerCase().includes(normalizedQuery) ||
            c.nameFr.toLowerCase().includes(normalizedQuery) ||
            c.nameAr.includes(query));
        results.push(...matchingCities);
        // Search regions
        const matchingRegions = this.regions.filter((r) => r.name.toLowerCase().includes(normalizedQuery) ||
     ...
     */
    async getRegionGeoJSON(regionCode) {
        const region = this.getRegion(regionCode);
        if (!region)
            return null;
        return {
     ...
     */
    async getAllLocationsGeoJSON() {
        const regionFeatures = this.regions.map((region) => ({
            type: "Feature",
            id: region.code,
            geometry: region.geometry || {
                type: "Point",

...
function assertArrayNotEmpty<T>(array: T[], message: string): void {
  if (!Array.isArray(array) || array.length === 0) {
    throw new Error(
      `Assertion failed: ${message} - array is empty or not an array`,
    );
  }
}
...
async function testCache(): Promise<void> {
  ...
  await test("Cache should handle different types", async () => {
    ...
    assertEquals(await cache.get<number>("number"), 42, "Number caching");
    assertEquals(
      await cache.get<object>("object"),
      { foo: "bar" },
      "Object caching",
    );
    assertEquals(
      await cache.get<number[]>("array"),
      [1, 2, 3],
      "Array caching",
    );
  });
...
async function testArabicUtilities(): Promise<void> {
  ...
  await test("normalizeArabic should normalize Ta Marbuta", async () => {
    assertEquals(
      normalizeArabic("فاطمة"),
      "فاطمه",
      "Ta marbuta should become ha",
    );
  });
  ...
  await test("expandSearchQuery should expand terms", async () => {
    ...
    assert(
      variants.some((v) => v.includes("المغرب")),
      "Should include Arabic variants",
    );
  });
}
...
async function testCKANClient(): Promise<void> {
  console.log("\n--- CKAN Client Tests ---");
...
async function testBAMClient(): Promise<void> {
  console.log("\n--- BAM Client Tests ---");
...
async function testWorldBankClient(): Promise<void> {
...
async function testBVCClient(): Promise<void> {
  console.log("\n--- BVC Client Tests ---");
...
async function testPrayerClient(): Promise<void> {
  console.log("\n--- Prayer Client Tests ---");
  ...
  await test("Prayer client should list cities", async () => {
    ...
    assert(
      cities.some((c) => c.key === "rabat"),
      "Should include Rabat",
    );
    assert(
      cities.some((c) => c.key === "casablanca"),
      "Should include Casablanca",
    );
  });
  ...
  await test("Prayer client should search cities", async () => {
    ...
    assert(
      results.some((r) => r.key === "casablanca"),
      "Should include Casablanca",
    );
  });
...
async function testIntegration(): Promise<void> {
  ...
  await test("Should get prayer times for multiple cities", async () => {
    ...
    const prayers = await Promise.all(
      cities.map((city) =>
        defaultPrayerClient.getToday(city).catch(() => null),
      ),
    );
